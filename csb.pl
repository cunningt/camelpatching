...
$ENV{JAVA_HOME} = "/opt/homebrew/Cellar/openjdk@21/21.0.8/libexec/openjdk.jdk/Contents/Home";
...
$currentprodbranch = "camel-spring-boot-4.14.2-branch";
...
# Change the version
system("export JAVA_HOME=/opt/homebrew/Cellar/openjdk\@21/21.0.8/libexec/openjdk.jdk/Contents/Home; /usr/local/apache-maven-3.9.9/bin/mvn -DnewVersion=${vers}-SNAPSHOT -DgenerateBackupPoms=false versions:set");

system("git commit -a -m \"Change versions to ${vers}-SNAPSHOT\"");

system("cp ../csb-rewrite.yml ./rewrite.yml");
system("export JAVA_HOME=/opt/homebrew/Cellar/openjdk\@21/21.0.8/libexec/openjdk.jdk/Contents/Home; /usr/local/apache-maven-3.9.9/bin/mvn -N -U org.openrewrite.maven:rewrite-maven-plugin:run -Drewrite.activeRecipes=com.redhat.camel-spring-boot.AddProductBranchProperties");
...
system("export JAVA_HOME=/opt/homebrew/Cellar/openjdk\@21/21.0.8/libexec/openjdk.jdk/Contents/Home; /usr/local/apache-maven-3.9.9/bin/mvn org.l2x6.cq:cq-camel-spring-boot-prod-maven-plugin:camel-spring-boot-prod-excludes -N");
...
system("export JAVA_HOME=/opt/homebrew/Cellar/openjdk\@21/21.0.8/libexec/openjdk.jdk/Contents/Home; /usr/local/apache-maven-3.9.9/bin/mvn -DskipTests clean install");
...
system ("cp -r ../$prodlocation/tooling/pom.xml ./tooling");
